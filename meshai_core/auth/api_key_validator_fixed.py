"""
MeshAI Shared API Key Validation Library - Fixed Version
Fixed to handle missing Redis gracefully
"""
...
class MeshAIAPIKeyValidator:
    """
    Shared API key validator for all MeshAI services - Fixed Version
    """
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Initialize Redis client only if URL is provided and Redis is available
            if self.redis_url and self.enable_caching and REDIS_AVAILABLE:
                try:
                ...
                if not REDIS_AVAILABLE:
                    ...
                    logger.info("No Redis URL configured, using local cache only")
        ...
        if self.redis_client:
            try:
                cached_data = await asyncio.wait_for(
                    ...
                    timeout=1  # 1 second timeout for Redis operations
                )
                if cached_data:
                    self.stats["cache_hits"] += 1
                ...
                logger.debug(f"Redis cache error (non-fatal): {e}")
    ...
    async def _set_cache(self, api_key: str, result: ValidationResult):
        ...
        if self.redis_client:
            try:
                ...
                logger.debug(f"Failed to set Redis cache (non-fatal): {e}")
    ...
    async def invalidate_cache(self, api_key: str):
        ...
        if self.redis_client:
            try:
                ...
                logger.debug(f"Failed to invalidate Redis cache (non-fatal): {e}")
